...
// RIDER CAN SEARCH FOR DRIVERS' CAR TYPE AND/OR COLOR
router.get('/:riderId', async(req, res) => {
    ...
    if (req.query.type && req.query.color) {
        for (let i = 0 ; i < allDrivers.length; i++) {
            if (allDrivers[i].carType === req.query.type && allDrivers[i].carColor === req.query.color) {
                foundDrivers.push(allDrivers[i]);
            }
        }
    }
    else if (req.query.type) {
        for (let i = 0 ; i < allDrivers.length; i++) {
            if (allDrivers[i].carType === req.query.type) {
                foundDrivers.push(allDrivers[i]);
            }
        }
    }
        ...
        for (let i = 0 ; i < allDrivers.length; i++) {
            if (allDrivers[i].carColor === req.query.color) {
                foundDrivers.push(allDrivers[i]);
            }
        }
    }
    else {
        foundDrivers = allDrivers;
    }
